...
- **Guard the synchronous window.** `busy` only flips once `setStreamingTurn` commits, so `resumingRef` is what actually stops a second concurrent run (the realistic case being "unlock the phone, immediately tap Send").
