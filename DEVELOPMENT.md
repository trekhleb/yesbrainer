...
- `npm run test:visual` — Playwright visual snapshots (`tests/visual/`): a regression net *and* a browsable inventory of every important screen/state, at desktop 1440×900 + mobile 390×844 viewports, against committed baselines in `tests/visual/__screenshots__/`. Fully offline: fake keys + a fixture bundle injected through the app's own zod-validated import. After a deliberate UI change, `npm run test:visual:update` refreshes baselines; `npm run test:visual:gallery` builds a contact sheet. Strategy in [`tests/visual/README.md`](./tests/visual/README.md). Not in CI — baselines are platform-suffixed and font rasterisation differs per machine, so a hosted runner would diff against baselines it didn't produce.
- `npm run test:integration` — Playwright behaviour tests (`tests/integration/`): full council runs — composer submit → AI SDK adapter → `fetch` → streaming → IndexedDB — with only the provider's HTTP response replaced (`page.route` on `api.anthropic.com`, answering in Anthropic's own wire format). Served from `dist/` via `vite preview` rather than the dev server, because the meta-CSP is injected at build only: a `connect-src` mistake breaks the app totally in production and is invisible to every other suite. Strategy in [`tests/integration/README.md`](./tests/integration/README.md).
...
- **Test the load-bearing parts, each at its own layer.** New logic ships with a unit test; new screen states ship with a visual baseline; a change to how a turn actually reaches a provider — adapters, request shape, error paths, CSP, persistence — ships with an integration spec. Don't duplicate assertions across suites: the unit suite owns logic, the visual suite owns pixels, the integration suite owns the wiring between them.
- **Readable > clever; no premature abstraction.** Optimize for the next reader; wait for the third use case before extracting.
- **The app is live — existing users' data is a hard constraint.** People already have councils in their browser, and there is no backend to migrate them from: the only copy of a user's history is on their device, so a bad read path destroys data nobody can restore. Every change to a persisted shape must leave old rows readable and old councils rendering exactly as before, with new behaviour arriving only for new data.
  - **Prefer additive, optional, non-indexed fields.** Adding one to a Dexie row needs no `version()` bump (`pos`, `isDemo`, `deliberation`, `runState` all landed this way). Avoid schema version bumps unless genuinely unavoidable: a tab still running a previously-cached bundle declares the *old* version and IndexedDB rejects the open outright (`VersionError`), so a routine deploy can hand a user a storage failure instead of an update.
  - **Normalize at the read boundary, degrade rather than throw.** Rows are written by whatever build was running at the time and are never re-validated on read (`normalizeSocialStructure`, `normalizeSeatConfig`, `normalizeRunState`). An unrecognised value degrades to the safest inert default — and "safest" means the direction that can't spend the user's money or lose their work.
  - **Absence must mean the old behaviour.** A missing new field has to read as "how this worked before", never as an unset flag that changes what the app does to existing data.
  - **Keep export/import round-tripping.** New fields either survive the round trip or are stripped deliberately at both ends; the import schema drops unknown keys, so anything left in an export that import discards silently breaks the round-trip contract.
  - If a real migration ever is unavoidable, it runs forward-only inside a Dexie `upgrade()`, is idempotent, and ships with a test that opens a fixture of the *old* shape.
...
- **`EMPTY_TOKENS`-aware reducers.** Subtract the previous total and add the new one — don't re-sum every event. The one exception is the orchestrator's own completion patch, which re-sums the turns (`summarizeTurns`): a turn is now persisted several times over its life, and whether the in-memory mirror already counted a given checkpoint depends on whether the page reloaded in between.
- **A run must survive being interrupted.** See "Interrupted runs" below — the invariant every phase upholds is *persisted event = done, absent = never happened*.
...
A council run is a chain of live provider streams that can last minutes, and the browser can take it away at any moment: **iOS suspends a locked phone within seconds**, and desktop discards backgrounded tabs. The web offers no way around that — a page gets no execution while the screen is locked, and every escape hatch is closed (Service Workers die with the page; Background Fetch and Background Sync are Chromium-only; Web Push needs a server this app doesn't have). So the design goal is not "keep running" but **"lose nothing, and never look broken"**.

- **A turn row exists before the first provider call.** The question itself survives a kill during the opening fan-out, and the composer draft can still clear on send.
- **Checkpoint at every phase boundary** — after the answer fan-out, each Mediator round, each re-answer fan-out, and voting. Not mid-stream: a partial completion can't be resumed at the provider anyway, so the finest *useful* granularity is a completed call.
- **`Turn.runState` present ⟺ the turn is unfinished.** The final persist carries none, and `appendTurn` treats its absence as authoritative. That single fact drives the paused card, the sidebar dot, the resume, and `isTurnShareable`'s refusal (a debate stopped at round 1 of 3 is not a verdict).
- **An interruption is not an error.** `utils/session/interruption.ts` classifies transport-level failures; an interrupted call persists **no event at all**, and that empty slot is exactly what tells the resume to re-issue it. This is what keeps "errored" (the user retries deliberately) distinct from "pending" (the app re-runs) without a second bookkeeping field to drift.
- **Resume is the same code path as a fresh run.** `executeTurn` takes `existingEvents`; `readTurnProgress` turns them into "already done"; each phase replays what it finds and issues only what it doesn't. An empty set reproduces the original behaviour exactly — there is deliberately no separate resume implementation, because it would only ever run when something had already gone wrong.
- **Liveness is decided by ownership, not the heartbeat.** `utils/session/run-lock.ts` (Web Locks) — the browser releases a dead page's lock with no timeout to tune, and it serialises resumes across tabs. `heartbeatAt` is informational only ("paused 4 minutes ago"): a hidden tab's timers are throttled to ~1/min, so a stale heartbeat would libel a healthy background run.
- **Auto-resume fires on foregrounding and on load, never on the interruption itself.** A run that dies in front of the user died because the network did; re-issuing a second later just spends another attempt and their tokens. `MAX_AUTO_RESUME_ATTEMPTS` bounds the rest; past it, only the Resume button remains.
- **Guard the synchronous window.** `busy` only flips once `setStreamingTurn` commits, so `resumingRef` is what actually stops a second concurrent run (the realistic case being "unlock the phone, immediately tap Send"). It is shared with the retry hooks for the same reason.
- **A run outlives its view, so the view must ask.** `CouncilView` is keyed by council id: navigating away and back gives a council a fresh hook with empty phase state while the earlier run carries on. `hasBackgroundRun` reads the run registry so the paused card reports that run instead of offering a Resume that could only bounce off its lock. The registry tracks **turn** ids as well as council ids precisely for this — the council-level set also counts the titler, which belongs to no turn.
- **Never pin-to-top on a resume.** The pin exists for a question the user just asked; a resumed turn is one they are already reading, usually from the paused card at the bottom. `chat-thread` tells them apart by whether the streaming turn is already in the mirror.

Known and accepted: a resumed debate is not byte-identical to an uninterrupted one — a re-issued round samples fresh, the same property the retry paths already have. And the tokens spent on the call that was cut off are gone; providers bill generation, not delivery.
...
- **New outbound endpoint?** Extend the CSP `connect-src` allowlist in `vite.config.ts` deliberately — never a wildcard. If the endpoint isn't a model provider the user chose, it doesn't belong in this app (no CDNs, no font hosts, no third-party telemetry) — the single carve-out is the self-hosted analytics collector (env-injected so forks default to none; closed pageview payload; the full disclosure lives in `SECURITY.md`). A new provider is build-asserted via `src/providers/endpoints.ts`, and `tests/integration/03-key-boundary.spec.ts` re-checks the shipped policy in a real browser — that the built document still carries the meta-CSP, that it names every wired origin and no wildcard, and that a live run reaches the chosen provider and nothing else.
