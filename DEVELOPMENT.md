...
**The OpenRouter slice is generated.** Native providers are hand-maintained in `registry.ts`; every valid model returned by OpenRouter's public catalog is emitted into `registry.generated.ts` by `npm run update-models-catalog` (capabilities are derived and the snapshot is committed, so the browser never fetches the catalog at runtime). Every generated entry has `provider: 'openrouter'`, including models developed by Anthropic, OpenAI, Google, and Groq; provider dispatch therefore reads the stored OpenRouter key for those entries, while the separate native entries keep their native-key routes. Native `providerModelId`s are bumped by hand. The generated file is data-only and is the explicit exception to the small-module line limit; catalog logic stays in the generator.
