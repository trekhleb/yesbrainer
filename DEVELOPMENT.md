...
- `npm run build` — `tsc -b` + Vite build + `scripts/spa-fallback.mjs`, which emits two extra files: `dist/404.html` (the GitHub Pages SPA fallback for per-user paths — served with an HTTP 404 status, so crawlers rightly skip them) and `dist/about.html` (Pages serves it at `/about` with a 200 — extensionless lookup, no redirect — with its own title/description/canonical/OG tags stamped in, because `/about` is the one public, contentful route and social crawlers don't execute JS). Deployed by `.github/workflows/deploy.yml` on push to `main`.
