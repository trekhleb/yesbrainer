...
test('new council: "Smartest available" preset seats one flagship per provider', async ({
  ...
  await expect(roster.getByText('Claude Fable 5.1')).toBeVisible()
  // Loose match on the generation, not the variant: the preset seats
  // whichever GPT-6 model the registry ranks highest (Astra today), so a
  // within-generation reshuffle shouldn't fail this. Bump when the
  // generation does.
  await expect(roster.getByText('GPT-6', { exact: false })).toBeVisible()
